...
export default function RelatorioPrintPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="no-print border-4 border-black bg-[#F4F4EB] p-3 sm:p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-3 sm:gap-4">
        <div>
          ...
          <h1 className="font-serif text-xl sm:text-2xl font-black uppercase break-words">
            RELATÓRIO CLÍNICO PARA CONSULTA MÉDICA
          </h1>
          <p className="font-mono text-xs sm:text-sm uppercase mt-0.5">
        ...
        <button
          ...
          className="w-full md:w-auto min-h-[48px] px-6 sm:px-8 bg-[#1A4331] text-[#F4F4EB] font-mono text-sm sm:text-base font-black uppercase border-2 border-black hover:bg-black hover:text-[#F4F4EB] transition-none text-center shrink-0"
        >
      ...
      <div className="bg-white text-black p-3.5 sm:p-6 md:p-10 border-4 border-black font-mono text-xs sm:text-sm leading-relaxed space-y-6">
        {/* Cabeçalho Oficial */}
        ...
        <section className="border-2 border-black p-3 sm:p-4 grid grid-cols-1 sm:grid-cols-3 gap-2">
          <div>
        ...
        <section className="space-y-2">
          <h3 className="font-serif text-base sm:text-lg font-black uppercase border-b-2 border-black pb-1 break-words">
            1. HORMONIOTERAPIA ADJUVANTE &amp; ADESÃO FARMACOLÓGICA
          </h3>
          <div className="w-full overflow-x-auto no-scrollbar -mx-1 px-1">
            <table className="w-full min-w-[320px] border-collapse border-2 border-black text-left">
              <tbody>
                <tr className="border-b border-black">
                  <th className="p-2 border-r border-black font-bold uppercase w-1/3 bg-[#F4F4EB] text-xs sm:text-sm">FÁRMACO PRESCRITO:</th>
                  <td className="p-2 font-black text-xs sm:text-sm">{dadosRelatorio.medicacao.farmaco}</td>
                </tr>
                <tr className="border-b border-black">
                  <th className="p-2 border-r border-black font-bold uppercase bg-[#F4F4EB] text-xs sm:text-sm">POSOLOGIA REGISTRADA:</th>
                  <td className="p-2 text-xs sm:text-sm">{dadosRelatorio.medicacao.dosagem}</td>
                </tr>
                <tr className="border-b border-black">
                  <th className="p-2 border-r border-black font-bold uppercase bg-[#F4F4EB] text-xs sm:text-sm">FASE DO TRATAMENTO:</th>
                  <td className="p-2 text-xs sm:text-sm">{dadosRelatorio.medicacao.periodo}</td>
                </tr>
                <tr className="border-b border-black">
                  <th className="p-2 border-r border-black font-bold uppercase bg-[#F4F4EB] text-xs sm:text-sm">ÍNDICE DE ADESÃO (RWD):</th>
                ...
                <tr>
                  <th className="p-2 border-r border-black font-bold uppercase bg-[#F4F4EB] text-xs sm:text-sm">ESTOQUE FÍSICO RESTANTE:</th>
                  <td className="p-2 font-black text-[#7C2D3A] text-xs sm:text-sm">{dadosRelatorio.medicacao.estoqueRestante}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <section className="space-y-2">
          ...
          <div className="w-full overflow-x-auto no-scrollbar -mx-1 px-1">
            <table className="w-full min-w-[500px] border-collapse border-2 border-black text-left">
              <thead>
                <tr className="border-b-2 border-black bg-[#F4F4EB]">
                  <th className="p-2 border-r border-black font-bold uppercase text-xs">SINTOMA CLÍNICO</th>
                  <th className="p-2 border-r border-black font-bold uppercase text-xs">SISTEMA</th>
                  <th className="p-2 border-r border-black font-bold uppercase text-xs">FREQUÊNCIA</th>
              ...
              <tbody>
                {dadosRelatorio.sintomasRecorrentes.map((s, idx) => (
                  <tr key={idx} className="border-b border-black last:border-0">
                    <td className="p-2 border-r border-black font-bold text-xs sm:text-sm">{s.sintoma}</td>
                    <td className="p-2 border-r border-black text-xs">{s.codigoLOINC}</td>
                    <td className="p-2 border-r border-black font-black text-center text-xs sm:text-sm">{s.ocorrencias90Dias}X</td>
        ...
        <section className="space-y-2">
          ...
          <div className="border-2 border-black p-3 sm:p-4 space-y-2 bg-[#F4F4EB]">
        ...
        <section className="space-y-2">
          ...
          <div className="w-full overflow-x-auto no-scrollbar -mx-1 px-1">
            <table className="w-full min-w-[450px] border-collapse border-2 border-black text-left">
              <thead>
                <tr className="border-b-2 border-black bg-[#F4F4EB]">
                  <th className="p-2 border-r border-black font-bold uppercase text-xs">EXAME / PROCEDIMENTO</th>
                  <th className="p-2 border-r border-black font-bold uppercase text-xs">DATA</th>
              ...
              <tbody>
                {dadosRelatorio.examesRealizadosPeriodo.map((ex, idx) => (
                  <tr key={idx} className="border-b border-black last:border-0">
                    <td className="p-2 border-r border-black font-bold text-xs sm:text-sm">{ex.exame}</td>
                    <td className="p-2 border-r border-black text-xs sm:text-sm">{ex.data}</td>
        ...
        <section className="border-2 border-black p-3 sm:p-4 space-y-5">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3 pt-3">
            <div className="text-xs break-words">
              ASSINATURA E CARIMBO (CRM): <span className="inline-block border-b border-black w-40 sm:w-56 align-bottom"></span>
            </div>
